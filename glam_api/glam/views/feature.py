            ...
            if geom['geometry']['type'] == 'Polygon' or geom['geometry']['type'] == 'MultiPolygon':
                ...
                if cropmask:
                    ...
                    with COGReader(mask_path) as mask_src:
                        ...
                        mean = float(Decimal(
                            str(data.mean())) * Decimal(str(product_dataset.product.variable.scale)))
                        _min = float(
                            Decimal(str(data.min())) * Decimal(str(product_dataset.product.variable.scale)))
                        _max = float(
                            Decimal(str(data.max())) * Decimal(str(product_dataset.product.variable.scale)))
                        stdev = float(
                            Decimal(str(data.std())) * Decimal(str(product_dataset.product.variable.scale)))
                ...
                if anomaly_type:
                    ...
                    mean = float(
                        Decimal(str(mean)) * Decimal(str(product_dataset.product.variable.scale)))
                    _min = float(
                        Decimal(str(_min)) * Decimal(str(product_dataset.product.variable.scale)))
                    _max = float(
                        Decimal(str(_max)) * Decimal(str(product_dataset.product.variable.scale)))
                    stdev = float(
                        Decimal(str(stdev)) * Decimal(str(product_dataset.product.variable.scale)))
