...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        operation_id="boundary-features"))
class BoundaryFeatureViewSet(viewsets.ViewSet):

    @swagger_auto_schema(
        manual_parameters=[boundary_layer_param]
    )
    def retrieve(self, request, layer_id: str = None):
        """
        Retrieve features belonging to a specified boundary layer.
        """
        boundary_features = BoundaryFeature.objects.all().prefetch_related(
            'boundary_layer'
        ).filter(boundary_layer__layer_id=layer_id).values(
            'feature_id', 'feature_name', 'boundary_layer'
        )
