...
def array_to_png(img_data: Array,
    ...
    if img_data.ndim == 3:  # encode RGB image
        ...
        if colormap is not None:
            raise ValueError('Colormap argument cannot be given for '
                             'multi-band data')
        ...
        if colormap is None:
            ...
            if isinstance(colormap, str):
                ...
                if colormap_array.ndim != 2 or colormap_array.shape[1] != 4:
                    raise ValueError('Explicit color mapping must have '
                                     'shape (n, 4)')
    ...
    sio = BytesIO()
    img.save(
        sio, 'png', compress_level=compress_level, transparency=transparency)
    sio.seek(0)
    return sio
