...
from .features import getWindows
from ..models import (
    Product,
    ProductRaster,
    BoundaryLayer,
            ...
            try:
                ...
                try:
                    BoundaryRaster.objects.get(
                        boundary_layer=boundary_layer, product=product
                    )
                    pass
            ...
            if filename.endswith(".tif"):
                ...
                try:
                    ds_date = datetime.datetime.strptime(
                        f"{parts[1]}.{parts[2]}", "%Y.%j"
                    ).strftime("%Y-%m-%d")
                except:
                    ds_date = datetime.datetime.strptime(parts[1], "%Y-%m-%d").strftime(
                        "%Y-%m-%d"
                    )
...
def upload_new_anomaly_baselines():
    ...
    for dataset in tqdm(anomaly_baselines):
        file_date = datetime.date.fromtimestamp(os.stat(dataset.local_path).st_mtime)
...
def upload_baselines_by_date(product_name, date):
    try:
        product_ds = ProductRaster.objects.get(product__name=product_name, date=date)
...
def create_median_raster(datasets, length, doy):
    start = datetime.datetime.now()
    end = datetime.datetime.now()
    print(end - start)


def create_mean_raster(datasets, length, doy):
    start = datetime.datetime.now()

    # n_cores = int(settings.N_CORES)
    # block_scale_factor = int(settings.BLOCK_SCALE_FACTOR)

    product = datasets[0].product
    ...
    if product.product_id in ["chirps-precip", "copernicus-swi"]:
        filename = f"{product.product_id}.{l}.mean.{doy}.tif"
    else:
        filename = f"{product.product_id}.{l}.mean.{str(doy).zfill(3)}.tif"
    directory = os.path.join(settings.ANOMALY_BASELINE_LOCAL_PATH, product.product_id)
    output = os.path.join(directory, filename)

    raster_data = [rasterio.open(ds.local_path) for ds in datasets]
...
def update_baseline(product_id, new_date=None):
    product = Product.objects.get(product_id=product_id)

    rasters = ProductRaster.objects.filter(product=product, prelim=False).order_by(
        "-date"
    )
    if new_date:
        ds = rasters.get(date=new_date)
    else:
        ds = rasters.first()

    print(ds.date)

    baseline_lengths = [0, 5, 10]
    outputs = []
    for length in baseline_lengths:
        if length > 0:
            ago = ds.date - relativedelta(years=length)
            new_rasters = rasters.filter(date__gt=ago)
        else:
            new_rasters = rasters
        ...
        if product_id in ["chirps-precip", "copernicus-swi"]:
            parts = ds.local_path.split(".")
            year, month, day = parts[1].split("-")
            doy = month + "-" + day
        else:
            doy = ds.date.timetuple().tm_yday

        for r in new_rasters:
            if product_id in ["chirps-precip", "copernicus-swi"]:
                ...
                new_doy = r.date.timetuple().tm_yday
                if new_doy == doy:
                    ds_list.append(r)
        print(length, len(ds_list))
        if product_id in ["chirps-precip", "copernicus-swi"]:
            baseline_average = create_mean_raster(ds_list, length, doy)
        else:
            baseline_average = create_mean_raster(ds_list, length, doy)

        outputs.append(baseline_average)
    print(outputs)
...
def create_boundary_raster(
    product_id, layer_id, feature_id_field_name, feature_func=None
):
    import os
    from django.conf import settings
    import rasterio
                ...
                for file in os.scandir(level.path):
                    ...
                    if ext == ".json":
                        ...
                        created = datetime.datetime.strptime(
                            metadataJSON["buildUpdateDate"], "%b %d, %Y"
                        ).date()
